...
app.post('/api/signup', async (req, res) =>{
    ...
    //Check user
    var sqlCheckUser = "select count(id) as count from users where username = '" + req.body[0].username + "';";
    connection.query(sqlCheckUser, (err, results) =>{
        ...
        if(checkUser > 0){
            ...
            //Insert new user
            var sql = "INSERT INTO `users`(`username`, `email`, `gender`, `wins`, `friends`, `password`)" +
                         "VALUES ('"+ username +"', '"+ email +"', '"+ gender +"', 0, '[]', '"+ hashedPassword.toString() +"')";
...
app.post('/api/signin', async (req, res) =>{
    requestedUsername = req.body[0].username;
    requestedPassword = req.body[0].password;
    const user = { username: requestedUsername};
    var sql = "select count(id) as count, username, password  from users where username = '" + requestedUsername + "';";
        ...
        if(checkLogin > 0){
            try{
                if(await bcrypt.compare(requestedPassword, userDataPassword)){
                    const accessToken = jwt.sign(user, process.env.ACCESS_TOKEN_SECRET, {expiresIn: 60 * 60 * 12});
